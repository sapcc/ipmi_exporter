...
GO_LDFLAGS = -X github.com/prometheus-community/ipmi_exporter.VERSION=$(shell git rev-parse --verify HEAD | head -c 8)
GO_TESTENV =

build-all: build/ipmi_exporter

build/ipmi_exporter: FORCE
	go build $(GO_BUILDFLAGS) -ldflags '-s -w $(GO_LDFLAGS)' -o build/ipmi_exporter ./
...
install: FORCE build/ipmi_exporter
	install -D -m 0755 build/ipmi_exporter "$(DESTDIR)$(PREFIX)/bin/ipmi_exporter"

# which packages to test with "go test"
GO_TESTPKGS := $(shell go list -f '{{if or .TestGoFiles .XTestGoFiles}}{{.ImportPath}}{{end}}' ./...)
# which packages to measure coverage for
GO_COVERPKGS := $(shell go list ./... | command grep -E '/pkg' | command grep -Ev '/test')
...
help: FORCE
	...
	@printf "  \e[36mbuild/ipmi_exporter\e[0m   Build ipmi_exporter.\n"
